...
describe('WorkflowService', () => {
  ...
  beforeEach(() => {
    ...
    workflowService = new WorkflowService(url, subscriptionClient)
    // subscription query
    query = gql`
        query {
          workflows {
  ...
  describe('recompute', () => {
    ...
    it('should not add duplicate callbacks', () => {
      const callback = new WorkflowCallback(store)
      for (const uid of ['view1', 'view2']) {
        workflowService.subscribe(uid, new SubscriptionQuery(
          query,
          subscriptionQuery.variables,
      ...
      // Only the one callback should be added to the subscription
      expect(subscription.callbacks).to.deep.equal([callback])
    })
    it('should add new callbacks', () => {
      const baseCallback = new WorkflowCallback(store)
      subscriptionQuery.callbacks.push(baseCallback)
      const newCallback = new TreeCallback(store)
      workflowService.subscribe('anotherView', new SubscriptionQuery(
        query,
        subscriptionQuery.variables,
        subscriptionQuery.name,
        { callbacks: [newCallback], runGlobalCallback: true }
      ))
      expect(subscription.callbacks).to.deep.equal([baseCallback, newCallback])
    })
