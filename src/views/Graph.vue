    ...
    >
      ...
      >
        ...
        <g
          ...
          "
        >
        <!-- TODO: do not use index for key below! -->
          <g
...
/**
 * Return true if the target is NOT intersecting the root element.
 *
 * @param {HTMLElement} target - The target element.
 * @param {HTMLElement} root - The root element that contains the target.
 * @param {number} threshold - If the target is only intersecting by this
 * many pixels or less, it is considered as NOT intersecting.
 */
function notIntersecting (target, root, threshold = 10) {
  const targetRect = target.getBoundingClientRect()
  const rootRect = root.getBoundingClientRect()
  return (
    targetRect.bottom - rootRect.top < threshold ||
    rootRect.bottom - targetRect.top < threshold ||
    targetRect.right - rootRect.left < threshold ||
    rootRect.right - targetRect.left < threshold
  )
}
...
export default {
  ...
  mounted () {
    ...
    const startAutoRefresh = () => {
      stopAutoRefresh = this.$watch(
        'hash',
        throttle(this.refresh, 2000),
        { immediate: true }
      )
    }
  ...
  computed: {
    ...
    hash () {
      return nonCryptoHash(
        ...
        this.edges.reduce((acc, { id }) => acc + id, '')
      )
    },
  },
  ...
  methods: {
    mountSVGPanZoom () {
      ...
      this.panZoomWidget = svgPanZoom(
        this.$refs.container,
        {
          ...
          viewportSelector: this.$refs.graph,
          panEnabled: true,
    ...
    reset () {
      ...
      this.panZoomTo(this.$refs.graph)
    },
    panZoomTo (ele) {
    ...
    /** Re-render the graph layout */
    async refresh () {
      console.log(`Refreshing graph ${this.hash}`)
      if (this.updating) {
      ...
      const { edges, cycles } = this
      // wipe old nodes
      this.graphNodes = this.nodes

      // obtain the node dimensions to use in the layoutcontainer
      // NOTE: need to wait for the nodes to all be rendered before we can
      // measure them
      await this.$nextTick()
      const nodeDimensions = this.getNodeDimensions(this.graphNodes)

      // layout the graph
      try {
        await this.layout(this.graphNodes, edges, nodeDimensions, cycles)
      } catch (e) {
      ...
      // was changed
      if (this.recenter || notIntersecting(this.$refs.graph, this.$refs.container)) {
        this.reset()
        this.recenter = false
      }
